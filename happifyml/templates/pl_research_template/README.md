...
[<kbd>Use this template</kbd>](https://github.com/thomas-happify/pl_research_template/generate) to initialize new DL repo.
...
- [Guide](#ℹ%EF%B8%8Fguide)

- [Rapid Experiments](#your-superpowers)
...
- [Start your README](#your-project-name)
...
```yaml
# clone project
git clone https://github.com/thomas-happify/pl_research_template
...
docker run --gpus all --ipc=host-it --rm <project_name>
```

## 🛠 Workflow

1. Write your PyTorch Lightning model (see [sample_model.py](src/models/sample_model.py) for example)
2. Write your PyTorch Lightning datamodule (see [sample_datamodule.py](src/datamodules/sample_datamodule.py) for example)
...
# reload pytorch-lightning model checkpoint
checkpoint: null

# save huggingface model by calling .save_pretrained()
save_hf: True
```
...
**[Aim](https://github.com/aimhubio/aim) · [Comet](https://www.comet.ml/) · [MLFlow](https://mlflow.org) · [Neptune](https://neptune.ai/) · [Tensorboard](https://www.tensorflow.org/tensorboard/) · [Weights&Biases](https://www.wandb.com/)**

To use, simply complete its configuration in [configs/logger](configs/logger) and run:

 ```yaml
 python run.py logger=logger_name
 ```
You can use many of them at once (see [configs/logger/multi_loggers.yaml](configs/logger/multi_loggers.yaml) for example).
...
```yaml
# this will return <you-sweep-id> something looks like `tuomasiii/xlmr-base/abc2heqs`
wandb sweep configs/hparams_search/wandb.yaml
...
Docker makes it easy to initialize the whole training environment, e.g. when you want to execute experiments in cloud or on some private computing cluster. You can extend [dockerfiles](https://github.com/thomas-happify/pl_research_template/tree/dockerfiles) provided in the template with your own instructions for building the image.<br>
...
Available [here](https://pytorch-lightning.readthedocs.io/en/latest/starter/style_guide.html).<br>
...
setup(
    ...
    url="https://github.com/thomas-happify/pl_research_template",
...
│   │
│   ├── tokenizer.py            <- Customized tokenizer wrapper if needed
...
<a href="https://github.com/thomas-happify/pl_research_template"><img alt="Template" src="https://img.shields.io/badge/-Lightning--Hydra--Template-017F2F?style=flat&logo=github&labelColor=gray"></a><br>
...
# move all data into data/ folder
mkdir data
mv <datasets> ./data
```

Build and run Docker (Skip if u already built docker from the template)

```yaml
# build docker image
docker build -t <project_name> .

# start docker with all GPUs
docker run --gpus all --ipc=host -ti --rm <project_name>

# start docker with specific GPUs
docker run --gpus '"device=2,3"' --ipc=host -ti --rm <project_name>
```
